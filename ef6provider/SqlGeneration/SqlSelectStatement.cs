...
namespace Ingres
{
  ...
  internal sealed class SqlSelectStatement : ISqlFragment
  {
    ...
    public void WriteSql(SqlWriter writer, SqlGenerator sqlGenerator)
    {
      ...
      if (Top != null && Skip == null)  // " FIRST " <TopCount>
      {
        // SELECT FIRST <TopCount> cannot be used in the same SELECT
        // statement as the OFFSET <SkipCount> clause, so defer (Top, Skip)
        // to OFFSET <SkipCount> FETCH NEXT <TopCount> ROWS ONLY.
        // FIRST <n> is only supported on the outermost SELECT,
        // else error "'FIRST n' clause is only allowed at the outermost level
        // of a SELECT query, not in subselects, view definitions, or CTE's.".
        // The TopMost checks will be done at the end of this method
        Top.WriteSql(writer, sqlGenerator); // " FIRST " <TopCount>
      }
      ...
      if ((null != this.orderBy)  &&  // write the ORDER BY unless inner SELECT
      ...
      if (this.Skip != null) // OFFSET <SkipCount>
      {                      //    FETCH NEXT <TopCount> ROWS ONLY
        this.Skip.WriteSql(writer, sqlGenerator);
        if (this.Top != null)
        {
          writer.Write(" FETCH NEXT ");
          this.Top.TopCount.WriteSql(writer, sqlGenerator);
      ...
      if (IsTopMost == false)  // if inner query, disallow Top and Skip
      {                        // because not supported by inner query
           if (Top  != null)
               throw new System.NotSupportedException(
                   "Queryable methods First() and Take() are not supported and "+
                   "cannot be mapped into a FIRST <topCount> "+
                   "clause of an SQL subselect.");
           if (Skip != null)
               throw new System.NotSupportedException(
                   "Queryable method Skip() is not supported and " +
                   "cannot be mapped into an OFFSET <skipCount> "+
                   "clause of an SQL subselect.");
      }  // end if (IsTopMost == false) // if in inside inner query
