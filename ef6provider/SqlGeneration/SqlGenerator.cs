...
namespace Ingres
{
  ...
  /// <summary>
  /// Translates the command object into a SQL string that can be executed.
  /// </summary>
  /// <remarks>
  ...
  {
    private IngresProviderManifest _manifest;
...
//#endif
    ...
    /// Basic constructor.
    /// </summary>
    internal SqlGenerator(IngresProviderManifest manifest)
    {
      _manifest = manifest;
    }
    #endregion
    ...
    internal static string GenerateSql(IngresProviderManifest manifest, DbCommandTree tree, out List<DbParameter> parameters, out CommandType commandType)
    {
      ...
      {
        return DmlSqlGenerator.GenerateInsertSql(insertCommandTree, sqlGen, out parameters);
      }
      ...
      {
        return DmlSqlGenerator.GenerateDeleteSql(deleteCommandTree, sqlGen, out parameters);
      }
      ...
      {
        return DmlSqlGenerator.GenerateUpdateSql(updateCommandTree, sqlGen, out parameters);
      }
      ...
      if (MetadataHelpers.TryGetPrimitiveTypeKind(e.ResultType, out typeKind))
      {
        switch (typeKind)
        {
          ...
          case PrimitiveTypeKind.SByte:
            result.Append(Format("int1({0})", e.Value));
            break;
          ...
          case PrimitiveTypeKind.Time:
            ...
            if (o.GetType() != typeof(TimeSpan))
                throw new NotSupportedException("time value is not TimeSpan");
            TimeSpan ts = (TimeSpan)e.Value;
            result.Append(String.Format("interval '{0} {1:00}:{2:00}:{3:00}.{4:000}' day to second(3)",
    ...
    public override ISqlFragment Visit(DbParameterReferenceExpression e)
    {
      ...
      result.Append(
          _manifest.CastParameterName(e.ResultType, "@" + e.ParameterName));
      return result;
    }
...
//#if USE_INTEROP_DLL && INTEROP_EXTENSION_FUNCTIONS
    /// <summary>
    ///  Function rename IndexOf -> POSITION
    /// </summary>
    /// <param name="sqlgen"></param>
    /// <param name="e"></param>
    /// <returns></returns>
...
//#endif
    ...
    /// <summary>
    /// We use the normal delimited identifier quotes for Ingres
    /// i.e. double quotes.
    /// </summary>
    ...
    void WriteFunctionName(SqlBuilder result, EdmFunction function)
    {
      ...
      else
      {
        ...
        //result.Append(QuoteIdentifier((string)function.MetadataProperties["Schema"].Value ?? "ingres"));
        //result.Append(".");
